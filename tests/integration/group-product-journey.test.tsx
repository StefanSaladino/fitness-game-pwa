  ...
  recentPrs: [
    {
      ...
      bestWeightKg: 100,
      bestReps: 4,
      achievedAt: "2026-08-19T22:30:00Z",
    },
  ],
  leaderboard: [
    {
...
function createMemoryGroupService(
  ...
  const service: GroupService = {
    ...
    async renameGroup(_groupId, name) {
      groups = groups.map((group) => ({ ...group, name }));
    },
    async revokeInvite(inviteId) {
      invites = invites.map((invite) =>
        invite.id === inviteId
          ? { ...invite, revokedAt: "2026-08-19T21:00:00Z" }
          : invite,
      );
    },
    async setMemberRole(_groupId, targetUserId, role) {
      members = members.map((member) =>
        member.userId === targetUserId ? { ...member, role } : member,
      );
    },
    async removeMember(_groupId, targetUserId) {
...
function OnboardingToGroupHarness({ service }: { service: GroupService }) {
  ...
  const [nextProfile, setNextProfile] = useState<OnboardingProfile>({
    ...profile,
    username: "u_pending",
...
describe("group-to-product integration journey", () => {
  it("moves from completed onboarding directly into persisted group setup", async () => {
    ...
    expect(
      await screen.findByRole("heading", {
        name: "Build your lifting identity.",
      }),
    ).toBeInTheDocument();
    await user.type(
  ...
  it("creates a group, enters the real dashboard, then administers the crew", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "115 XP this week" }),
    ).toBeInTheDocument();
    expect(screen.getByText("Barbell Bench Press")).toBeInTheDocument();
    expect(dashboardService.load).toHaveBeenCalledWith(
  ...
  it("joins with a full invite URL and reaches the member dashboard without admin controls", async () => {
    ...
    await user.type(
      ...
      `https://app.example.com/join/${INVITE_TOKEN}`,
    );
    ...
    expect(
      screen.queryByRole("button", { name: "New invite" }),
