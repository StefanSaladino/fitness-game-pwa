...
interface MemoryOptions { initialGroups?: GroupSummary[]; pendingInvites?: PendingGroupInvite[] }
function createMemoryGroupService(options:MemoryOptions={}){
 ...
 for(const group of groups){membersByGroup.set(group.id,[{userId:USER_ID,username:'stefan',displayName:'Stefan',profilePicturePath:null,profilePictureUrl:null,role:group.role,joinedAt:group.joinedAt}]);}
 let createdCount=0;
 const service:GroupService={
  ...
  async createGroup(_userId,input){
    ...
    const created:GroupSummary={id,name:input.name,memberCount:1,role:'OWNER',joinedAt:'2026-08-23T20:00:00Z',createdAt:'2026-08-23T20:00:00Z'};
    groups=[...groups,created];
    membersByGroup.set(id,[{userId:USER_ID,username:'stefan',displayName:'Stefan',profilePicturePath:null,profilePictureUrl:null,role:'OWNER',joinedAt:created.joinedAt}]);
    return {...created};
  },
  async getMembers(groupId){return (membersByGroup.get(groupId)??[]).map(member=>({...member}));},
  async createInvite(_userId,groupId,recipient){const invite:GroupInvite={id:'invite-out',groupId,invitedUserId:TEAMMATE_ID,invitedUsername:recipient.replace(/^@/,''),invitedDisplayName:'Alex',createdAt:'2026-08-23T20:10:00Z'};outgoing=[...outgoing,invite];return {...invite};},
  async joinByInvite(){throw new Error('Reusable invite codes are no longer supported.');},
  async listInvites(groupId){return outgoing.filter(invite=>invite.groupId===groupId).map(i=>({...i}));},
  async listPendingInvites(){return pending.map(i=>({...i}));},
  async acceptInvite(id){
    const invite=pending.find(item=>item.id===id); if(!invite) throw new Error('Invite missing');
    pending=pending.filter(item=>item.id!==id);
    const joined:GroupSummary={id:invite.groupId,name:invite.groupName,memberCount:2,role:'MEMBER',joinedAt:'2026-08-23T20:15:00Z',createdAt:'2026-08-20T20:00:00Z'};
    if(!groups.some(group=>group.id===joined.id)) groups=[...groups,joined];
    membersByGroup.set(joined.id,[
      {userId:TEAMMATE_ID,username:'alex',displayName:'Alex',profilePicturePath:null,profilePictureUrl:null,role:'OWNER',joinedAt:'2026-08-20T20:00:00Z'},
      {userId:USER_ID,username:'stefan',displayName:'Stefan',profilePicturePath:null,profilePictureUrl:null,role:'MEMBER',joinedAt:joined.joinedAt},
    ]);
    return joined.id;
  },
  ...
  async removeMember(groupId,targetUserId){membersByGroup.set(groupId,(membersByGroup.get(groupId)??[]).filter(m=>m.userId!==targetUserId));},
...
const dashboardService:DashboardService={load:vi.fn(async(input)=>({...snapshot,leaderboard:input.groupId?snapshot.leaderboard:[]}))};

function JourneyHarness({service,initialProfile=profile}:{service:GroupService;initialProfile?:OnboardingProfile}){
 return <GroupGate service={service} userId={USER_ID}>{(groups,refreshGroups)=><ProductController dashboardService={dashboardService} groupService={service} groups={groups} onGroupsChanged={refreshGroups} profile={initialProfile} progressService={progressService} socialService={socialService}/>}</GroupGate>;
}

function OnboardingToProductHarness({service}:{service:GroupService}){
 ...
 if(!completed)return <OnboardingScreen busy={false} profile={nextProfile} onSubmit={async(input:OnboardingInput)=>{setNextProfile(current=>({...current,username:input.username,displayName:input.displayName,timezone:input.timezone,weeklyWorkoutTarget:input.weeklyTarget,onboardingCompletedAt:'2026-08-23T20:00:00Z'}));setCompleted(true);return true;}}/>;
 return <JourneyHarness initialProfile={nextProfile} service={service}/>;
}

const firstInvite:PendingGroupInvite={id:'invite-in',groupId:GROUP_ID,groupName:'Night Crew',invitedByUserId:TEAMMATE_ID,invitedByUsername:'alex',invitedByDisplayName:'Alex',createdAt:'2026-08-23T18:00:00Z'};
const secondInvite:PendingGroupInvite={id:'invite-second',groupId:SECOND_GROUP_ID,groupName:'Weekend Crew',invitedByUserId:TEAMMATE_ID,invitedByUsername:'alex',invitedByDisplayName:'Alex',createdAt:'2026-08-23T19:00:00Z'};
const existingGroup:GroupSummary={id:GROUP_ID,name:'Iron Crew',memberCount:1,role:'OWNER',joinedAt:'2026-08-20T20:00:00Z',createdAt:'2026-08-20T20:00:00Z'};
...
describe('group-to-product integration journey',()=>{
 it('moves from completed onboarding directly into the personal dashboard with zero groups',async()=>{
  const user=userEvent.setup();const service=createMemoryGroupService();render(<OnboardingToProductHarness service={service}/>);
 ...
 it('lets a solo user accept a targeted invitation from the dashboard when they choose',async()=>{
  const user=userEvent.setup();const service=createMemoryGroupService({pendingInvites:[firstInvite]});render(<JourneyHarness service={service}/>);
  expect(await screen.findByRole('heading',{name:'Your lifting week'})).toBeInTheDocument();
 ...
 it('lets a solo user create a group later from Groups without blocking personal training first',async()=>{
  const user=userEvent.setup();const service=createMemoryGroupService();render(<JourneyHarness service={service}/>);
  expect(await screen.findByRole('heading',{name:'Your lifting week'})).toBeInTheDocument();
  await user.click(screen.getAllByRole('button',{name:'Groups'})[0]!);
 ...
 it('keeps memberships additive when an existing owner accepts another invitation',async()=>{
  const user=userEvent.setup();const service=createMemoryGroupService({initialGroups:[existingGroup],pendingInvites:[secondInvite]});render(<JourneyHarness service={service}/>);
  expect(await screen.findByRole('heading',{name:'Your lifting week'})).toBeInTheDocument();
 ...
 it('still keeps group competition and personal progress behind their proper product boundaries',async()=>{
  const user=userEvent.setup();const service=createMemoryGroupService({initialGroups:[existingGroup]});render(<JourneyHarness service={service}/>);
