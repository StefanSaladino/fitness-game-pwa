...
function createMemoryGroupService(mode:'create'|'invited'|'owned-member'='create'):GroupService{
 ...
 const teammateMember:GroupMember={...teammateOwner,role:'MEMBER'};
 const currentMember:GroupMember={...ownerMember,role:'MEMBER'};
 return {
  async listGroups(){return groups.map(g=>({...g}));},
  async createGroup(_userId,input){const id=groups.length===0?GROUP_ID:SECOND_GROUP_ID;const initialMembers=mode==='owned-member'&&groups.length===0?[ownerMember,teammateMember]:[ownerMember];const g:GroupSummary={id,name:input.name,memberCount:initialMembers.length,role:'OWNER',joinedAt:'2026-08-19T20:00:00Z',createdAt:'2026-08-19T20:00:00Z'};groups=[...groups,g];membersByGroup.set(id,initialMembers);return {...g};},
...
describe('group-to-product integration journey',()=>{
 ...
 it('accepts a targeted invitation into a member dashboard without admin controls',async()=>{const user=userEvent.setup();const groupService=createMemoryGroupService('invited');const dashboardService:DashboardService={load:vi.fn(async()=>snapshot)};render(<JourneyHarness dashboardService={dashboardService} service={groupService}/>);expect(await screen.findByRole('heading',{name:'Your lifting week'})).toBeInTheDocument();await user.click(screen.getAllByRole('button',{name:'Groups'})[0]!);expect(await screen.findByText('Night Crew')).toBeInTheDocument();await user.click(screen.getByRole('button',{name:'Accept'}));expect(await screen.findByRole('heading',{name:'Members'})).toBeInTheDocument();expect(screen.queryByRole('textbox',{name:'Username or invite ID'})).not.toBeInTheDocument();expect(screen.getByRole('button',{name:'Leave group'})).toBeInTheDocument();await user.click(screen.getAllByRole('button',{name:'Home'})[0]!);expect(await screen.findByRole('heading',{name:'Your lifting week'})).toBeInTheDocument();expect(dashboardService.load).toHaveBeenCalledWith(expect.objectContaining({groupId:GROUP_ID,userId:USER_ID}));});

 it('keeps targeted invite and Make admin administration scoped to the selected owned group',async()=>{const user=userEvent.setup();const groupService=createMemoryGroupService('owned-member');await groupService.createGroup(USER_ID,{name:'Iron Crew'});const dashboardService:DashboardService={load:vi.fn(async()=>snapshot)};render(<JourneyHarness dashboardService={dashboardService} service={groupService}/>);await screen.findByRole('heading',{name:'Your lifting week'});await user.click(screen.getAllByRole('button',{name:'Groups'})[0]!);expect(await screen.findByRole('button',{name:'Make admin'})).toBeInTheDocument();await user.click(screen.getByRole('button',{name:'Make admin'}));await user.type(screen.getByRole('textbox',{name:'Username or invite ID'}),'@alex');await user.click(screen.getByRole('button',{name:'Send invite'}));const invitesSection=await screen.findByRole('region',{name:'Invites'});expect(within(invitesSection).getByText('@alex')).toBeInTheDocument();});
});
