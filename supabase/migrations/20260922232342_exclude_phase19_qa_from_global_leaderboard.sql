-- Reconcile the hosted Phase 19 QA leaderboard exclusion migration.
-- This migration was already applied to hosted Supabase as 20260922232342.
-- It excludes synthetic Phase 19 QA accounts from the global all-time
-- leaderboard without deleting their workouts, scoring events, or progress.

create table if not exists private.global_leaderboard_exclusions (
...
revoke all on table private.global_leaderboard_exclusions
from public, anon, authenticated;

insert into private.global_leaderboard_exclusions (
  user_id,
  reason
)
select
...
begin
  ...
  with eligible_users as (
    ...
    where p.onboarding_completed_at is not null
      ...
      and not exists (
        select 1
        from private.global_leaderboard_exclusions gle
        where gle.user_id = p.id
      )
  ), scoring as (
    select
      u.id as user_id,
      coalesce(
        sum(se.amount) filter (where se.scoring_version = 'lifting-v1'),
        0
      )::bigint as xp,
  ...
  from output o
  order by
    case when o.row_kind = 'TOP' then 0 else 1 end,
    o.global_rank,
    o.member_user_id;
end;
$$;

revoke all on function public.get_global_all_time_leaderboard()
from public, anon, authenticated;

grant execute on function public.get_global_all_time_leaderboard()
to authenticated;

comment on function public.get_global_all_time_leaderboard() is
  'Global lifetime leaderboard excluding explicitly registered synthetic QA accounts.';
