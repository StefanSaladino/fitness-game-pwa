...
type ProfileRow={id:string;username:string;display_name:string;timezone:string;weekly_workout_target:number;pending_weekly_workout_target:number|null;onboarding_completed_at:string|null;profile_code:string};
...
export function createOnboardingService(client:SupabaseClient=getSupabaseClient()):OnboardingService{return{
 async getProfile(userId){const {data,error}=await client.from('profiles').select('id, username, display_name, timezone, weekly_workout_target, pending_weekly_workout_target, onboarding_completed_at, profile_code').eq('id',userId).single();if(error)throw error;if(!data)throw new Error('Profile not found.');const r=data as ProfileRow;return{id:r.id,username:r.username,displayName:r.display_name,timezone:r.timezone,weeklyWorkoutTarget:r.weekly_workout_target,pendingWeeklyWorkoutTarget:r.pending_weekly_workout_target,onboardingCompletedAt:r.onboarding_completed_at,profileCode:r.profile_code};},
