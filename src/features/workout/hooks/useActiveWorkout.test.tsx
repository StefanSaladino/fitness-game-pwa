...
function service(overrides: Partial<WorkoutService> = {}): WorkoutService {
  return {
    ...
    pauseWorkout: vi.fn(async () => ({
      ...
      pausedAt: '2026-08-19T22:02:00.000Z',
      lastResumedAt: null,
    })),
    resumeWorkout: vi.fn(async () => ({
      ...active,
      activeDurationSeconds: 120,
      lastResumedAt: '2026-08-19T22:03:00.000Z',
    })),
...
describe('useActiveWorkout', () => {
  it('recovers an existing active session on mount', async () => {
    const api = service({
      loadActiveWorkout: vi.fn(async () => active),
    });
  ...
  it('forwards exact pause/resume button timestamps to the service', async () => {
    ...
    await act(async () => {
      await result.current.pause(1_000);
    });

    expect(api.pauseWorkout).toHaveBeenCalledWith(
      'workout-1',
      1_000,
    );

    await act(async () => {
      await result.current.resume(2_000);
    });

    expect(api.resumeWorkout).toHaveBeenCalledWith(
      'workout-1',
      2_000,
    );
  });
  ...
  it('forwards ordered preset ids and the exact start timestamp through the same lifecycle state', async () => {
    ...
    await act(async () => {
      await result.current.startPreset(
        ['exercise-1', 'exercise-2'],
        4_000,
      );
    });

    expect(api.startPresetWorkout).toHaveBeenCalledWith(
      ['exercise-1', 'exercise-2'],
      4_000,
      [],
    );
