...
const secondGroup: GroupSummary = {
  id: 'group-2',
  name: 'Sunday Crew',
  memberCount: 5,
  role: 'MEMBER',
...
const allTime: GroupCompetitionLeaderboard = {
  period: 'ALL_TIME',
  periodStart: null,
  periodEnd: null,
  entries: [
...
const feed: GroupSocialFeedItem[] = [{
  ...
  reactions: { FIRE: 2, STRONG: 0, CLAP: 0 },
  myReaction: null,
}];

describe('GroupSocialScreen', () => {
  it('shows privacy-safe competition, switches groups and periods, and delegates reactions', async () => {
  ...
  it('offers quiet report controls for other users and attaches the visible social activity', async () => {
    const user = userEvent.setup();
    const reportService: UserReportService = {
      submit: vi.fn().mockResolvedValue({ caseId: 'case-id' }),
    };
    ...
    await user.type(
      ...
      'This visible group activity needs moderator review.',
    );
