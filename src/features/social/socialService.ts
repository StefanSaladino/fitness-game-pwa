...
import type { BadgeActivityMetadata, GoalActivityMetadata, GroupCompetitionLeaderboard, GroupCompetitionPeriod, GroupReactionType, GroupSocialActivityMetadata, GroupSocialActivityType, GroupSocialFeedCursor, GroupSocialFeedItem, GroupSocialFeedPage, LiftActivityMetadata, PrActivityMetadata } from './model';
...
type LeaderboardRow = { rank:number|string; member_user_id:string; username:string; display_name:string; profile_picture_path:string|null; xp:number|string; lifting_days:number|string; pr_count:number|string; badge_count:number|string; is_current_user:boolean; period_start:string|null; period_end:string|null };
...
export interface GroupSocialService {
  loadLeaderboard(groupId:string, period:GroupCompetitionPeriod):Promise<GroupCompetitionLeaderboard>;
...
export function createGroupSocialService(client:SupabaseClient=getSupabaseClient()):GroupSocialService{
  ...
  return {
    async loadLeaderboard(groupId,period){const result=await client.rpc('get_group_competition_leaderboard',{p_group_id:groupId,p_period:period});if(result.error)throw result.error;const rows=(result.data??[]) as LeaderboardRow[];return {period,periodStart:rows[0]?.period_start??null,periodEnd:rows[0]?.period_end??null,entries:rows.map(row=>({rank:Math.max(1,integerValue(row.rank)),userId:row.member_user_id,username:row.username,displayName:row.display_name,profilePictureUrl:publicPictureUrl(row.profile_picture_path),xp:integerValue(row.xp),liftingDays:integerValue(row.lifting_days),prCount:integerValue(row.pr_count),badgeCount:integerValue(row.badge_count),isCurrentUser:row.is_current_user}))};},
