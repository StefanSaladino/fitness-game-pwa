...
type WorkoutRow = { id: string; subtype: string | null; scoring_date: string; started_at: string; active_duration_seconds: number };
...
type ProgressRow = { exercise_id: string; metric_type: DashboardPrMetric; best_value: number | string; best_weight_kg: number | string | null; best_reps: number | null; achieved_at: string };
type ExerciseRow = { id: string; canonical_name: string };
type ProfilePictureRow = { profile_picture_path: string | null };
type LeaderboardRow = { member_user_id: string; username: string; display_name: string; profile_picture_path: string | null; xp: number | string };

export interface DashboardService { load(input: DashboardLoadInput, now?: Date): Promise<DashboardSnapshot> }
...
export function createDashboardService(client: SupabaseClient = getSupabaseClient()): DashboardService {
  ...
  return {
    async load(input, now = new Date()) {
      ...
      const leaderboardPromise = input.groupId
        ? client.rpc('get_group_lifting_leaderboard', { p_group_id: input.groupId, p_week_start: weekStart })
        : Promise.resolve({ data: [], error: null });

      const [weeklyEventsResult, recentWorkoutsResult, progressResult, profileResult, leaderboardResult] = await Promise.all([
        client.from('scoring_events').select('event_type, amount, workout_id, scoring_date').eq('user_id', input.userId).eq('scoring_version', 'lifting-v1').gte('scoring_date', weekStart).lte('scoring_date', weekEnd),
        client.from('workout_sessions').select('id, subtype, scoring_date, started_at, active_duration_seconds').eq('user_id', input.userId).eq('category', 'STRENGTH').eq('status', 'COMPLETED').order('started_at', { ascending: false }).limit(4),
        client.from('exercise_progress').select('exercise_id, metric_type, best_value, best_weight_kg, best_reps, achieved_at').eq('user_id', input.userId).order('achieved_at', { ascending: false }).limit(4),
        client.from('profiles').select('profile_picture_path').eq('id', input.userId).single(),
        leaderboardPromise,
      ]);
      ...
      const weeklySessionDates = weeklyEvents.filter((event) => event.event_type === 'LIFTING_WORKOUT' && typeof event.scoring_date === 'string').map((event) => event.scoring_date!);
      ...
      const [workoutExercisesResult, recentWorkoutEventsResult, exerciseNamesResult] = await Promise.all([
        workoutIds.length ? client.from('workout_exercises').select('workout_id, exercise_id').in('workout_id', workoutIds) : Promise.resolve({ data: [], error: null }),
        workoutIds.length ? client.from('scoring_events').select('event_type, amount, workout_id').eq('user_id', input.userId).eq('scoring_version', 'lifting-v1').in('workout_id', workoutIds) : Promise.resolve({ data: [], error: null }),
        progressExerciseIds.length ? client.from('exercise_catalog').select('id, canonical_name').in('id', progressExerciseIds) : Promise.resolve({ data: [], error: null }),
      ]);
      ...
      const recentLifts: DashboardRecentLift[] = recentWorkoutRows.map((workout) => ({
        id: workout.id, title: liftTitle(workout.subtype), scoringDate: workout.scoring_date, startedAt: workout.started_at,
        durationMinutes: Math.max(0, Math.round(workout.active_duration_seconds / 60)), exerciseCount: exerciseCountByWorkout.get(workout.id)?.size ?? 0, xp: xpByWorkout.get(workout.id) ?? 0,
      }));
      const recentPrs: DashboardRecentPr[] = progressRows.map((row) => ({
        exerciseId: row.exercise_id, exerciseName: exerciseNameById.get(row.exercise_id) ?? 'Exercise', metricType: row.metric_type,
        bestValue: numberValue(row.best_value) ?? 0, bestWeightKg: numberValue(row.best_weight_kg), bestReps: row.best_reps, achievedAt: row.achieved_at,
      }));
      ...
      const leaderboard: DashboardLeaderboardEntry[] = leaderboardRows.map((row, index) => ({
        rank: index + 1, userId: row.member_user_id, username: row.username, displayName: row.display_name,
        profilePictureUrl: publicPictureUrl(row.profile_picture_path), xp: Number(row.xp), isCurrentUser: row.member_user_id === input.userId,
      }));
      const currentProfilePicturePath = (profileResult.data as ProfilePictureRow | null)?.profile_picture_path ?? null;

      return {
        weekStart, weekEnd, weeklyTarget: resolvedWeeklyTarget, completedLiftingDays: uniqueScoringDayCount(weeklySessionDates),
        completedLiftingDates: [...new Set(weeklySessionDates)].sort(), weeklyXp: scoreSummary.weeklyXp, xpBreakdown: scoreSummary.breakdown,
        recentLifts, recentPrs, leaderboard, currentUserProfilePictureUrl: publicPictureUrl(currentProfilePicturePath), consistency,
      };
    },
  };
}
