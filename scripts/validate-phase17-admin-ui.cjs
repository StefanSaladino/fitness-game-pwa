...
ok(capacity.includes('Organization-level quotas'), 'capacity page must explain omitted organization-level quotas');
ok(!capacity.includes('Provider status'), 'corrected capacity page must not render provider-status pseudo-telemetry');
ok(capacity.includes('data-capacity-metric'), 'capacity cards require stable visual-test hooks');
...
console.log('Phase 17 admin UI validation passed: production CSS/TSX structure is container-driven and compatible with measurable-only capacity telemetry.');
